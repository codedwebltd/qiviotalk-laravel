    ...
    <form @submit.prevent="submitForm" class="pb-20">
      ...
      <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl transition-shadow duration-300 mb-6">
        ...
        <div class="p-4 sm:p-6 space-y-3">
          ...
          <div v-for="(item, index) in featureLimits" :key="index" class="flex gap-2">
            <input
              v-model="item.key"
              type="text"
            ...
            <button
              type="button"
              @click="removeFeatureLimit(index)"
              class="px-3 py-2 bg-red-50 text-red-600 text-xs rounded-lg hover:bg-red-100 transition-colors"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="featureLimits.length === 0" class="text-center py-8 text-gray-500">
...
const addFeatureLimit = () => {
  featureLimits.value.push({ key: '', value: '' })
}

const removeFeatureLimit = (index) => {
  if (!confirm('Remove this feature limit?')) return
  featureLimits.value.splice(index, 1)
}
...
const submitForm = () => {
  if (!confirm('Create new subscription plan?\n\nThis will:\n- Add a new plan to the system\n- Make it available based on active status\n- Apply configured feature limits')) {
    return
  }

  processing.value = true

  // Convert feature limits array to object
  const feature_limits = {}
  featureLimits.value.forEach(item => {
  ...
  router.post('/admin/subscriptions', data, {
    preserveScroll: true,
    onSuccess: () => {
